// Edge Function: get-internal-credential
...
Deno.serve(async (req: Request) => {
  ...
  if (password !== undefined && password !== null) {
    ...
    const { error } = await client
      ...
      .eq("id", id);
    if (error) return err("Failed to save credential", 400);
  ...
  const { data: row, error } = await client
    ...
    .eq("id", id)
    .single<{ password_encrypted: string | null }>();

  if (error) return err("Record not found", 404);
  if (!row?.password_encrypted) return json({ data: null });

  const plaintext = await decrypt(row.password_encrypted);
