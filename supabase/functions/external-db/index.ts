...
// Fixed org ID — single-tenant system (Koraflow internal use only).
...
// Signing fields a client is allowed to set when signing a contract.
const CONTRACT_SIGNING_FIELDS = new Set([
  "client_signed_at", "client_signature_data", "client_signer_name",
  "client_signer_email", "client_cpf", "status", "fully_signed_at",
  "signed_document_data", "document_data",
]);
...
Deno.serve(async (req: Request) => {
  ...
  // Public: contract select by signature token (no user session required)
  if (action === "select" && table === "contracts" && (filters as Record<string, unknown>)?.signature_link_token) {
    audit({ action, table, userId: null, ip, userAgent, outcome: "success" });
    const client = createClient(extUrl, extKey);
    return handleSelect(client, table, filters, pagination);
  }

  // Public: client lookup during contract signing — token must map to a real contract first
  if (action === "select" && table === "clients" && token) {
    const verifyClient = createClient(extUrl, extKey);
    const { data: contractRows, error: tokenErr } = await verifyClient
      .from("contracts")
      .select("id")
      .eq("signature_link_token", token)
      .limit(1);
    ...
    audit({ action, table, userId: null, ip, userAgent, outcome: "success" });
    return handleSelect(verifyClient, table, filters, pagination);
  }

  // Public: contract signing via signature link token (no user session required)
  if (action === "sign_contract") {
    if (!token) return err("token required for sign_contract", 400);
    if (!data) return err("data required for sign_contract", 400);
    audit({ action, table, userId: null, ip, userAgent, outcome: "success" });
    const client = createClient(extUrl, extKey);
