...
import { taskStatus, priority as priorityColors } from "@/lib/colors";
import { useInternalTasks } from "@/hooks/useInternalTasks";
import type { InternalTask, InternalTaskStatus } from "@/types/data";
...
const STATUS_CFG: Record<InternalTaskStatus, { label: string; bar: string; dot: string }> = {
...
const PRIORITY_CFG = {
  low:    { label: "Baixa",  color: priorityColors.low.dot },
  medium: { label: "Média",  color: priorityColors.medium.dot },
  high:   { label: "Alta",   color: priorityColors.high.dot },
};
...
export function EmpresaGantt({ workspaceId }: Props) {
  ...
  const sorted = useMemo(() => {
    const order: Record<InternalTaskStatus, number> = {
  ...
  function barStyle(task: InternalTask) {
    const start = parseDate(task.createdAt) ?? rangeStart;
    const end = parseDate(task.dueDate) ?? addDays(startOfDay(start), 3);
  ...
  if (tasks.length === 0) {
    return (
      <div className="text-center py-12 text-muted-foreground text-sm">
        Nenhuma tarefa cadastrada para exibir no Gantt.
      </div>
    );
  }
            ...
            {sorted.map((task, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate text-xs font-medium text-foreground leading-tight">{task.title}</p>
                </div>
                <span className={cn("w-1.5 h-1.5 rounded-full flex-shrink-0", PRIORITY_CFG[task.priority].color)} />
              </div>
            ))}
          </div>
              ...
              {sorted.map((task, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        "absolute h-6 rounded-md flex items-center px-2 z-20 cursor-default group shadow-sm",
                        isOverdue ? "bg-red-500/80" : STATUS_CFG[task.status].bar
                      )}
                      style={{ left: left + 2, width: Math.max(width - 4, 8) }}
                      title={`${task.title} — ${task.dueDate ?? "sem prazo"}`}
                    >
                      {width >= 48 && (
                      ...
                      {/* Tooltip */}
                      <div className="absolute bottom-full left-0 mb-1.5 hidden group-hover:flex bg-popover border border-border rounded-lg shadow-lg p-2.5 z-30 min-w-[180px] flex-col gap-1 pointer-events-none">
                        <p className="font-semibold text-foreground text-xs leading-tight">{task.title}</p>
                        <p className="text-muted-foreground text-[10px]">
                          Status: {STATUS_CFG[task.status].label}
                        </p>
                        <p className="text-muted-foreground text-[10px]">
                          Prioridade: {PRIORITY_CFG[task.priority].label}
                        </p>
                        {task.dueDate && (
                          <p className="text-muted-foreground text-[10px]">
                        ...
                        {isOverdue && (
                          <p className="text-red-500 text-[10px] font-medium">⚠ Atrasada</p>
                        )}
                      </div>
                    </div>
                  </div>
