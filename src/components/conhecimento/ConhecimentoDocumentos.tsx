...
export function ConhecimentoDocumentos() {
  ...
  const openEdit = (item: KnowledgeItem) => {
    ...
    else if (item.url && !item.content) contentMode = "link";
    setFormData({
      title: item.title,
    ...
    if (editingItem) {
      updateKnowledgeItem(
        editingItem.id,
        {
          ...
          content: isFileMode || isLinkMode ? "" : formData.content,
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem ? "Atualize os dados do documento." : "Adicione um novo documento de cliente."}
            </DialogDescription>
          </DialogHeader>
          <DialogBody className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setFormData((p) => ({ ...p, docType: v === "none" ? "" : v }))}
              >
                <SelectTrigger>
