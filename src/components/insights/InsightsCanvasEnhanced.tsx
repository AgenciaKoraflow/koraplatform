...
export function InsightsCanvasEnhanced({ board, onSave, onBack, onRename }: InsightsCanvasEnhancedProps) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-200px)] bg-card rounded-xl border border-border overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-4 p-3 border-b border-border bg-secondary/30">
        <button
          onClick={onBack}
      ...
      <div className="flex items-center gap-2 p-3 border-b border-border bg-secondary/30">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button className="p-2 rounded-lg hover:bg-secondary transition-colors flex items-center gap-2" title="Cor">
              <div className="w-5 h-5 rounded-md border border-border" style={{ backgroundColor: currentColor }} />
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => handleZoom(-0.1)} className="p-2 rounded-lg hover:bg-secondary transition-colors" title="Diminuir zoom">
            <ZoomOut className="w-4 h-4 text-muted-foreground" />
          </button>
          <span className="text-sm text-muted-foreground min-w-[50px] text-center">{Math.round(zoom * 100)}%</span>
          <button onClick={() => handleZoom(0.1)} className="p-2 rounded-lg hover:bg-secondary transition-colors" title="Aumentar zoom">
            <ZoomIn className="w-4 h-4 text-muted-foreground" />
          </button>
          <button onClick={handleReset} className="p-2 rounded-lg hover:bg-secondary transition-colors" title="Resetar visualização">
        ...
        {selectedId && (
          <>
            {selectedElement && (selectedElement.type === "sticky" || selectedElement.type === "rectangle" || selectedElement.type === "circle") && (
              <Popover>
                <PopoverTrigger asChild>
                  <button className="p-2 rounded-lg hover:bg-secondary transition-colors" title="Cor do elemento">
                    <div className="w-5 h-5 rounded-md border border-border" style={{ backgroundColor: selectedElement.color }} />
                  </button>
                </PopoverTrigger>
            ...
            <button onClick={handleDelete} className="p-2 rounded-lg hover:bg-destructive/10 text-destructive transition-colors" title="Excluir (Delete)">
        ...
        <div style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transformOrigin: "0 0" }}>
          ...
          {elements.map((el) => (
            ...
            >
              ...
              {el.type === "sticky" && (
                <div className="w-full h-full rounded-lg shadow-lg p-3 flex flex-col" style={{ backgroundColor: el.color }}>
                  {editingTextId === el.id ? (
                    <textarea
                      autoFocus
                      value={el.text || ""}
